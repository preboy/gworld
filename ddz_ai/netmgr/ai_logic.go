...
func (ai *AILogic) Init(c *connector, pos int32, arr []int32) {
...
func (ai *AILogic) CallScoreCalcBroadcast(draw bool, lord int32, score int32, arr []int32) {
	...
	if lord == ai.pos {
		...
		log.Info("%v 是我<地主>，我的最终牌: %v", pos_to_string(ai.pos), poker.CardsToString(ai.cards))
	}
...
func (ai *AILogic) on_play(pos int32, first bool, cards []poker.Card) {
	if first {
		ai.rounds = append(ai.rounds, &round{})
	}
...
func (ai *AILogic) is_lord() bool {
	return ai.pos == ai.pos_lord
}

func (ai *AILogic) get_friend_pos() int32 {
	if ai.is_lord() {
		panic("is lord")
	}

	for i := int32(0); i < 3; i++ {
		if i == ai.pos_lord || i == ai.pos {
			continue
		}
